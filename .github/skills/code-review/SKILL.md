...
- Maintenance automation runs only in `jaydestro/gallery`. Treat `AzureCosmosDB/gallery` as read-only.
- Generated catalog changes remain on `automation/gallery-content-updates` in a draft pull request. Automation must never approve, merge, or enable automerge.
